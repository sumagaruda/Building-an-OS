...
int main() {
 ...
 unsigned long n_info_frames = ContFramePool::needed_info_frames(PROCESS_POOL_SIZE);
    ...
    //Console::puts ("Identifying the problem");
   ...
   Console::puts("Everything is initialized properly");
...
void test_memory(ContFramePool * _pool, unsigned int _allocs_to_go) {
    ...
    if (_allocs_to_go > 0) {
        ...
        //Console::puts("No problem at get_frames");
        //Console::puti(l);
        ...
        //Console::puts("Are we reaching release_frames?");
